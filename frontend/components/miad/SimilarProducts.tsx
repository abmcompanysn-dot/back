"use client"

import { useEffect, useState } from 'react'
import { ProductCard } from './ProductCard'
import { type WooProduct } from '@/lib/woocommerce'

interface SimilarProductsProps {
  productId: number | string
  onProductClick: (product: WooProduct) => void
  onAddToCart: (product: WooProduct) => void
}

/**
 * "Produits similaires" — nearest-neighbors sémantiques (Vectorize + Workers
 * AI), basés sur le nom/catégorie/description du produit plutôt que sur des
 * co-achats. Complète FrequentlyBoughtTogether, ne le remplace pas. Reste
 * discret (rien affiché) si le produit n'a pas encore été embeddé (voir
 * node scripts/miad.mjs sync-embeddings).
 */
export function SimilarProducts({ productId, onProductClick, onAddToCart }: SimilarProductsProps) {
  const [products, setProducts] = useState<WooProduct[]>([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    let cancelled = false
    setLoading(true)
    fetch(`/api/products/${productId}/similar?limit=6`)
      .then((r) => (r.ok ? r.json() : { ids: [] }))
      .then(async (data) => {
        const ids: string[] = data.ids || []
        if (!ids.length) { if (!cancelled) setProducts([]); return }
        const results = await Promise.all(
          ids.map((id) => fetch(`/api/products?id=${id}`).then((r) => (r.ok ? r.json() : null)).catch(() => null))
        )
        if (cancelled) return
        setProducts(results.flatMap((r) => (r?.products?.[0] ? [r.products[0]] : [])))
      })
      .finally(() => { if (!cancelled) setLoading(false) })
    return () => { cancelled = true }
  }, [productId])

  if (loading || products.length === 0) return null

  return (
    <div className="mt-16 pt-10 border-t border-border">
      <h2 className="text-xl font-bold mb-6">Produits similaires</h2>
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">
        {products.map((p) => (
          <ProductCard key={p.id} product={p} onClick={onProductClick} onAddToCart={onAddToCart} />
        ))}
      </div>
    </div>
  )
}
