...
// order-svc ne stocke pas d'image produit sur la ligne de commande — on
// enrichit avec catalog-svc, même logique que /api/orders/[id]/confirm-paydunya.
...
/**
 * Confirme le paiement PawaPay en relisant directement le statut agrégé de
 * la commande (order-svc), déjà mis à jour de façon fiable par le webhook
 * serveur-à-serveur PawaPay (pawapayWebhook → re-vérification GET
 * /v2/deposits/{id} → payment-svc.confirmPayment → order-svc). On ne
 * rappelle PAS PawaPay depuis ici : la commande est déjà la source de
 * vérité fiable — même approche que confirm-paydunya / confirm-stripe.
 *
 * `id` est le PARENT order id (commande groupée). order-svc expose sa vue
 * agrégée sur GET /orders/parent/{id} (statut "paid" seulement si TOUTES
 * les sous-commandes sont payées).
 */
export async function POST(
  ...
  try {
    ...
    // MobileMoneyDirectForm.tsx affiche un message précis (ex. solde
    // insuffisant) plutôt qu'un "échec" générique, même cause pour toute
    // erreur. Best-effort : une erreur réseau ici ne doit pas empêcher
    // d'informer le client de l'échec lui-même.
    ...
    console.error('[confirm-pawapay]', e.message)
